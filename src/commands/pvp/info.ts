...
import { calcMul, StatModifier, statusTypes, teamNames } from "../../battle.js"
    ...
    async run(i: ChatInputCommandInteraction) {
        ...
        function statsString() {
            return Object.keys(player.modifiers).map(el => {
                ...
                return formatString(`${getString("stat." + el).padEnd(12, " ")} [a]${Math.floor(player.stats[el as StatID])}[r] ${dispDelta(Math.ceil(finalStats[el as StatID] - player.stats[el as StatID]), true)}`
